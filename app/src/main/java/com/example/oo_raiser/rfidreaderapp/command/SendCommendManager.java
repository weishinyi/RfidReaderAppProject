...
public class SendCommendManager implements CommendManager{
    ...
    public SendCommendManager(InputStream serialPortInput, OutputStream serialportOutput) {
        in = serialPortInput;
    ...
    public void setSensitivity(int value) {
        ...
        byte[] response = this.read();
        if (response != null) {
            //Log.e("setSensitivity ", Tools.Byte2HexString(response, response.length));
        }
    }
    ...
    private byte[] read() {
        ...
        try {
            while( cnt > 0){
                ...
                if(size > 0){
                    Log.e("buffer--", Tools.Byte2HexString(buffer, size));
                    count += size ;
                    if(count > 512){
                        count = 0;
    ...
    private byte[] readMemBank(int memBank, int startAddr, int length, byte[] accessPassword, int count){
        ...
        if (response != null) {
            ...
            if (resolve != null) {
                // BB 01 39 00 1F 0E 30 00 01 61 05 B8 03 48 0C D0 00 03 D1
